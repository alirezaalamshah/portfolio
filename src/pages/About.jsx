        ...
        <main className={`about-container ${isRTL ? 'rtl' : 'ltr'}`}>
          ...
          <section className="about-section">
            ...
            <div className="about-content">
              {Array.isArray(aboutContent) &&
                aboutContent.map((text, index) => <p key={index}>{text}</p>)}
            </div>
          </section>
          ...
          <section className="skills-section">
            ...
            <div className="skills-grid">
              {['programming_languages','frontend','backend','tools_technologies','design','soft_skills'].map(
                (category) => {
                  const items = t(`skills.${category}.items`, { returnObjects: true });
                  return (
                ...
                experienceItems.map((item, index) => (
                  <div className="timeline-item" key={index}>
                    <div className="timeline-date">
                      ...
                      {item.status && <span className="date-status">{item.status}</span>}
                    </div>
